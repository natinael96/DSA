def editDistance(str1, str2, n, m, insrtCost, deletCost, matchCost, misCost):
    ...
    for i in range(m + 1):
        for j in range(n + 1):
            if i == 0:
                dp[i][j] = j * deletCost  # Deletion cost
            elif j == 0:
                dp[i][j] = i * insrtCost  # Insertion cost
            else:
    ...
    print("   ", end="")  # Add space for the top-left corner
    for ch in str1:
        print(f" {ch} ", end="")
    print()
    ...
    align_str1 = ""
    align_str2 = ""
    i, j = m, n  # Start from the bottom-right corner
    while i > 0 or j > 0:
...
print("Aligned Sequence 1:", aligned_str1)
print("Aligned Sequence 2:", aligned_str2)
